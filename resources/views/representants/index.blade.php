            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Listes des représentants</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        @endif
                        @if($message = session('suppression'))
                        <div class="alert alert-danger alert-dismissible">
                            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                            <h5><i class="icon fas fa-check"></i> Alert!</h5>
                            {{ $message }}
                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped table-sm" style="font-size: 12px">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $compteur++ }}</td>
                                        <td><img class="profile-user-img img-fluid img-circle" style="height: 50px; width: 50px" @if ($representant->photo)
                                            src="{{ asset('images')}}/{{ $representant->photo }}"
                                            @else
                                            src="{{asset('dist/img/default.jpg')}}"
                                            @endif
                                            alt="Photo representant"></td>
                                        <td>{{ $representant->matricule }}</td>
                                        <td>{{ $representant->civilite }}</td>
                                        <td>{{ $representant->nom }}</td>
                                        <td>{{ $representant->prenom }}</td>
                                        <td>{{ $representant->telephone }}</td>
                                        <td>{{ $representant->telephonepro }}</td>
                                        <td>{{ $representant->email }}</td>

                                        @if(!(Auth::user()->roles()->where('libelle', ['CONTROLEUR'])->exists() || Auth::user()->roles()->where('libelle', ['VALIDATEUR'])->exists() || Auth::user()->roles()->where('libelle', ['SUPERVISEUR'])->exists()))
                                        <td class="text-center">
                                            <!--<a class="btn btn-success btn-sm" href=""><i class="fa-regular fa-eye"></i></a>-->
                                            <a class="btn btn-warning btn-sm" href="{{ route('representants.edit', ['representant'=>$representant->id]) }}"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <a class="btn btn-danger btn-sm" href="{{ route('representants.delete', ['representant'=>$representant->id]) }}"><i class="fa-solid fa-trash-can"></i></a>
                                        </td>
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-md-right dropdown-menu-icon-list drop text-sm">
                                                    <a class="dropdown-item" href="{{ route('representants.zone', ['representant'=>$representant->id]) }}"><i class="nav-icon fa-solid fa-earth-africa"></i> Zones</a>
                                                </div>
                                            </div>
                                        </td>
                                        @endif
