                        ...
                        <div class="card-body">
                            <table id="example1" class="table table-bordered table-striped table-sm" style="font-size: 12px">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $compteur++ }}</td>
                                        <td><img class="profile-user-img img-fluid img-circle" style="height: 50px; width: 50px" src="@if ($produit->photo)
                                                            {{ asset('images')}}/{{ $produit->photo }}
                                                        @else
                                                            {{asset('dist/img/ciment.jpg')}}
                                                        @endif" alt="Photo produit"></td>
                                        <td>{{ $produit->libelle }}</td>
                                        <td>{{ $produit->typeproduit->libelle }}</td>
                                        @if(!(Auth::user()->roles()->where('libelle', ['CONTROLEUR'])->exists() || Auth::user()->roles()->where('libelle', ['VALIDATEUR'])->exists() || Auth::user()->roles()->where('libelle', ['SUPERVISEUR'])->exists()))
                                        <td class="text-center">
                                            <a class="btn btn-success btn-sm" href="{{ route('produits.show', ['id'=>$produit->id]) }}"><i class="fa-regular fa-eye"></i></a>
                                            <a class="btn btn-warning btn-sm" href="{{ route('produits.edit', ['id'=>$produit->id]) }}"><i class="fa-solid fa-pen-to-square"></i></a>
                                            <a class="btn btn-danger btn-sm" href="{{ route('produits.delete', ['id'=>$produit->id]) }}"><i class="fa-solid fa-trash-can"></i></a>
